...
pub enum AppError {
    ...
    Redis {
        ...
        #[source] source: redis::RedisError
    },
    ...
    SessionSendFailed {
        user_id: String,
        #[source] source: SessionSendError
    },
...
impl AppError {
    ...
    pub fn to_client_message(&self) -> (&'static str, String) {
        match self {
            ...
            AppError::UnknownSocketType { value } => ("BAD_REQUEST", format!("Unknown opcode: {}", value)),
            ...
            AppError::DuplicateLogin { .. } => ("ALREADY_CONNECTED", "User is already connected".into()),
...
impl From<redis::RedisError> for AppError {
    fn from(source: redis::RedisError) -> Self {
        AppError::Redis { reason: "operation failed".into(), source }
    }
}
